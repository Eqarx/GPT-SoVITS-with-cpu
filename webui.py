...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    with gr.Tabs():
        with gr.TabItem(i18n("0-前置数据集获取工具")):#提前随机切片防止uvr5爆内存->uvr5->slicer->asr->打标
            ...
            with gr.Row():
                with gr.Row():
                    ...
                    slice_opt_root=gr.Textbox(label=i18n("切分后的子音频的输出根目录"),value="output/slicer_xxx")
            ...
            with gr.Row():
                ...
                path_list = gr.Textbox(
                    ...
                    value="output\\asr_opt\\slicer_xxx.list",
                    interactive=True,
                )
        ...
        with gr.TabItem(i18n("1-GPT-SoVITS-TTS")):
            with gr.Row():
                exp_name = gr.Textbox(label=i18n("*实验/模型名(最好与数据名相同(去掉‘slicer’))"), value="xxx", interactive=True)
            ...
            with gr.TabItem(i18n("1A-训练集格式化工具")):
                ...
                with gr.Row():
                    inp_text = gr.Textbox(label=i18n("*文本标注文件"),value=r"D:\AI\GPT-SoVITS\output\asr_opt\slicer_xxx.list",interactive=True)
                    inp_wav_dir = gr.Textbox(
                        label=i18n("*训练集音频文件目录"),
                        value=r"D:\AI\GPT-SoVITS\output\slicer_xxx",
                        interactive=True,
            ...
            with gr.TabItem(i18n("1B-微调训练")):
                ...
                with gr.Row():
                    batch_size = gr.Slider(minimum=1,maximum=64,step=1,label=i18n("每张显卡的batch_size"),value=16,interactive=True)
                    total_epoch = gr.Slider(minimum=1,maximum=96,step=1,label=i18n("总训练轮数total_epoch，不建议太高，16左右"),value=8,interactive=True)
                    text_low_lr_rate = gr.Slider(minimum=0.2,maximum=0.6,step=0.05,label=i18n("文本模块学习率权重"),value=0.4,interactive=True)
                    save_every_epoch = gr.Slider(minimum=1,maximum=96,step=1,label=i18n("保存频率save_every_epoch"),value=4,interactive=True)
                ...
                with gr.Row():
                    batch_size1Bb = gr.Slider(minimum=1,maximum=64,step=1,label=i18n("每张显卡的batch_size"),value=16,interactive=True)
                    total_epoch1Bb = gr.Slider(minimum=2,maximum=256,step=1,label=i18n("总训练轮数total_epoch，48左右"),value=15,interactive=True)
                    ...
                    save_every_epoch1Bb = gr.Slider(minimum=1,maximum=256,step=1,label=i18n("保存频率save_every_epoch"),value=5,interactive=True)
            ...
            with gr.TabItem(i18n("1C-推理")):
                gr.Markdown(value=i18n("选择训练完存放在SoVITS_weights和GPT_weights下的模型。"))
                with gr.Row():
